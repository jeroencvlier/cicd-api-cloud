...
async def exercise_function(data: IncomePrediction):
    ...
    if getattr(data, "age") < 0 or getattr(data, "fnlgt") < 0 or getattr(data, "education_num") < 0 or getattr(data, "capital_gain") < 0 or getattr(data, "capital_loss") < 0 or getattr(data, "hours_per_week") < 0:
        raise HTTPException(
